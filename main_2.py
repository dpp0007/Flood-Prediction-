"""
Flood Early Warning Prediction System - Demo Mode (main_2.py)

Complete end-to-end pipeline: Data → Clean → Features → Train → Predict → Alerts

DEMO MODE FEATURE:
- Identical to main.py in all aspects
- ADDITIONALLY: Forces MEDIUM and HIGH risk alerts for demonstration/testing
- If real MEDIUM/HIGH alerts exist: uses them (demo_mode = false)
- If NO real MEDIUM/HIGH alerts: artificially raises top stations (demo_mode = true)
- All forced alerts are transparently marked in JSON and logs

Data Source: Central Water Commission (CWC), Government of India
Website: https://ffs.india-water.gov.in/

Usage:
    python main_2.py                    # Run complete pipeline with demo alerts
    python main_2.py --data-file <path> # Use specific data file
"""
...
class DemoFloodPredictionPipeline:
    """
    Complete end-to-end flood prediction pipeline with demo alert-raising capability.
    ...
    def apply_demo_alerts(self, predictions: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
        """
        DEMO MODE: Apply forced MEDIUM/HIGH alerts if none exist naturally.
        
        Args:
            predictions: Predictions DataFrame
            
        Returns:
            Tuple of (modified_predictions, demo_mode_active)
        """
        logger.info("\n" + "="*70)
        logger.info("STEP 5.5: DEMO ALERT CHECK")
        logger.info("="*70)
        
        # Count real risk levels
        real_high = len(predictions[predictions['risk_tier_name'].str.lower() == 'high'])
        real_medium = len(predictions[predictions['risk_tier_name'].str.lower() == 'medium'])
        real_low = len(predictions[predictions['risk_tier_name'].str.lower() == 'low'])
        
        logger.info(f"Real risk distribution:")
        logger.info(f"  HIGH: {real_high}")
        logger.info(f"  MEDIUM: {real_medium}")
        logger.info(f"  LOW: {real_low}")
        
        # Check if demo mode needed
        if real_high == 0 and real_medium == 0:
            ...
            # Force 1 HIGH risk station
            if len(predictions_sorted) > 0:
                high_idx = predictions_sorted.index[0]
                predictions.loc[high_idx, 'risk_score'] = 0.75  # Force HIGH
                predictions.loc[high_idx, 'risk_tier'] = 2
                predictions.loc[high_idx, 'risk_tier_name'] = 'HIGH'
                predictions.loc[high_idx, 'demo_mode'] = True
                predictions.loc[high_idx, 'alert_mode'] = 'forced_high'
                self.forced_high_count = 1
                logger.info(f"[DEMO MODE] Forced HIGH alert for station: {predictions.loc[high_idx, 'station_id']}")
            
            # Force 2 MEDIUM risk stations
            if len(predictions_sorted) > 1:
                for i in range(1, min(3, len(predictions_sorted))):
                    med_idx = predictions_sorted.index[i]
                    predictions.loc[med_idx, 'risk_score'] = 0.55  # Force MEDIUM
                    predictions.loc[med_idx, 'risk_tier'] = 1
                    predictions.loc[med_idx, 'risk_tier_name'] = 'MEDIUM'
                    predictions.loc[med_idx, 'demo_mode'] = True
                    predictions.loc[med_idx, 'alert_mode'] = 'forced_medium'
                    self.forced_medium_count += 1
                    logger.info(f"[DEMO MODE] Forced MEDIUM alert for station: {predictions.loc[med_idx, 'station_id']}")
        else:
            logger.info("[DEMO MODE] Real MEDIUM/HIGH alerts detected. Using natural predictions.")
            self.demo_mode_active = False
            # Mark real alerts as not demo
            predictions['demo_mode'] = False
            predictions['alert_mode'] = 'natural'
        
        return predictions, self.demo_mode_active
    ...
    def run(self, csv_path: str = 'data/raw/cwc_hydrograph_20260102_050433.csv') -> Dict:
        """Run complete pipeline with demo alert capability."""
        logger.info("\n" + "="*70)
        logger.info("FLOOD EARLY WARNING PREDICTION SYSTEM - DEMO MODE (main_2.py)")
        logger.info("="*70)
        ...
        logger.info(f"Mode: DEMO (Forces MEDIUM/HIGH alerts for testing)")
        logger.info("="*70 + "\n")
        ...
        try:
            ...
            # DEMO MODE: Apply forced alerts if needed
            predictions, demo_active = self.apply_demo_alerts(predictions)
            results['demo_mode'] = demo_active
            results['forced_high_count'] = self.forced_high_count
            results['forced_medium_count'] = self.forced_medium_count
            ...
            logger.info(f"Demo Mode: {results['demo_mode']}")
            logger.info("="*70 + "\n")
    ...
    def save_results(self, predictions: pd.DataFrame, alerts: pd.DataFrame, 
        ...
        """Save pipeline results including JSON predictions with demo metadata."""
        timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        
        # Save predictions table (already done in generate_predictions)
        # Now convert to JSON with filtering
        logger.info("\n" + "="*70)
        logger.info("STEP 7: JSON CONVERSION (FILTERED + DEMO METADATA)")
        logger.info("="*70)
        ...
        self.output_manager.print_pipeline_summary(
            "JSON Predictions (Filtered + Demo)",
            {
                ...
                'Demo mode active': self.demo_mode_active,
                'Forced HIGH alerts': self.forced_high_count,
                'Forced MEDIUM alerts': self.forced_medium_count,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Flood Early Warning Prediction System - Demo Mode (main_2.py)"
    )
    ...
    if results['status'] == 'success':
        ...
        # Print demo alert summary
        print("\n" + "-"*60)
        print("MAIN_2 DEMO ALERT SUMMARY")
        print("-"*60)
        print(f"Real HIGH risk detected: {json_stats.get('high_risk_count', 0)}")
        print(f"Real MEDIUM risk detected: {json_stats.get('medium_risk_count', 0)}")
        print(f"DEMO MODE ACTIVE: {'YES' if results.get('demo_mode') else 'NO'}")
        print(f"Forced HIGH alerts: {results.get('forced_high_count', 0)}")
        print(f"Forced MEDIUM alerts: {results.get('forced_medium_count', 0)}")
        print(f"JSON files generated: {json_stats.get('json_generated', 0)}")
        print("-"*60 + "\n")
        
        # Print final summary
        print("\n" + "="*70)
        print("FINAL EXECUTION SUMMARY (DEMO MODE)")
        print("="*70)
        ...
        print(f"Demo Mode: {results.get('demo_mode')}")
        print(f"Forced Alerts: {results.get('forced_high_count', 0)} HIGH + {results.get('forced_medium_count', 0)} MEDIUM")
        print("="*70 + "\n")
